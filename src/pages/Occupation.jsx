...
export default function Occupation() {
  ...
  const [activeOccupationName, setActiveOccupationName] = useState(null);
  ...
  const filteredDevices = useMemo(() => {
    if (!activeOccupationName) return [];
    const activeUpper = activeOccupationName.trim().toUpperCase();
  ...
  }, [devices, activeOccupationName, deviceTypeFilter, deviceSearchQuery]);
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `devices_${activeOccupationName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
        ...
        <div className="w-full">
          <div className="bg-panel border border-main shadow-xl rounded-2xl overflow-hidden min-h-[400px]">
            <div className="p-6 border-b border-main bg-main flex items-center justify-between">
              {activeOccupationName ? (
                <div className="flex items-center space-x-4">
                  <button 
                    onClick={() => setActiveOccupationName(null)}
              ...
              {loading ? (
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-panel rounded-2xl p-4 border border-main gap-4">
                    <div>
                      <h3 className="text-sm font-black text-main uppercase tracking-widest flex items-center">
                        ...
                        Devices in {activeOccupationName}
                      </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {occupations.map(occ => (
                    <div 
                      key={occ.id} 
                      onClick={() => setActiveOccupationName(occ.name)}
                    ...
                    >
                      ...
                      <div className="flex items-start">
                        <div className="p-4 rounded-xl mr-5 bg-teal-500/10 text-teal-500 border border-teal-500/10 group-hover:scale-110 transition-transform">
                          <Building size={24} />
                        </div>
                        <div className="pt-1">
