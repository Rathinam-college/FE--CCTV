...
export default function SwitchDetail() {
  ...
  const fetchDetails = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/cameras/switches/${id}/`);
      setDevice(res.data);
      setFormData({
        name: res.data.name || '',
        ...
        portCount: res.data.portCount || '',
        status: res.data.status || 'Online'
      });
      setLoading(false);
    } catch (err) {
      console.error(err);
      setError('Failed to fetch switch details.');
      setLoading(false);
    }
  };
  ...
  const handleSaveRemarks = async (remarkText) => {
    ...
    try {
      ...
      const res = await api.post(`/cameras/switches/${id}/add_remark/`, { remark: textToSave });
      setDevice({ 
        ...device, 
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      ...
      await api.patch(`/cameras/switches/${id}/`, { status: newStatus });
      setDevice({ ...device, status: newStatus });
      showNotification(`Switch status transitioned to ${newStatus}`);
    } catch (err) {
      console.error(err);
  ...
  const handleSaveRelocation = async () => {
    try {
      ...
      await api.post(`/cameras/switches/${id}/add_relocation/`, {
      ...
      showNotification('Switch successfully relocated and logs updated');
      setEditMode(false);
      fetchDetails();
    } catch (err) {
      console.error(err);
      showNotification('Failed to relocate switch', 'error');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const downloadExcel = () => {
    const filename = `SWITCH_ASSET_${device.name || 'Export'}_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in pb-12 relative">
      ...
      <div className="hidden print-only-header">
        ...
        <div className="letter-sub text-black">Network Switching & Routing Protocol</div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 no-print">
        <div className="space-y-1">
          <nav className="flex items-center space-x-2 text-[10px] font-black uppercase tracking-[0.2em] text-dim">
            ...
            <button onClick={() => navigate('/network-switches')} className="hover:text-emerald-400 transition-colors">NETWORK</button>
            <ChevronRight size={10} />
            <span className="text-emerald-400">PORT ANALYSIS</span>
          </nav>
            ...
            <div className="glass-panel overflow-hidden border-white/10 shadow-2xl bg-card/30">
              <div className="p-5 bg-panel border-b border-white/10 flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h3 className="text-sm font-black text-main uppercase tracking-widest">Network Intelligence Registry</h3>
                    <p className="text-[9px] text-dim uppercase tracking-[0.3em] font-bold">Deep Hardware & Topology Signature</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-8 space-y-12">
                ...
                <section>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <DetailRow label="Static IPv4" value={device.ipAddress} mono />
                  </div>
                </section>
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-panel overflow-hidden border-emerald-500/20 shadow-xl flex flex-col bg-emerald-500/5">
            ...
            <div className="p-5 space-y-6">
               ...
               {unifiedActivity.length > 0 && (
                 <button 
                   onClick={() => navigate(`/devices/history/switch/${id}`)}
          ...
          <div className="glass-panel p-5 border-white/5 bg-white/[0.02] space-y-4">
             ...
             <div className="grid grid-cols-2 gap-3">
                <button 
                  onClick={() => navigate('/network-switches')}
      ...
      <style dangerouslySetInnerHTML={{ __html: `
            @media print {
              ...
              .border-emerald-500\\/10 { border-color: #000 !important; margin-top: 30px !important; margin-bottom: 15px !important; }
