...
export default function BiometricDetail() {
  const { id } = useParams();
  ...
  const fetchDetails = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/cameras/biometrics/${id}/`);
      setDevice(res.data);
      setFormData({
        name: res.data.name || '',
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      ...
      await api.patch(`/cameras/biometrics/${id}/`, { status: newStatus });
      setDevice({ ...device, status: newStatus });
      showNotification(`Device status transitioned to ${newStatus}`);
    } catch (err) {
      console.error(err);
  ...
  const handleSaveRelocation = async () => {
    try {
      ...
      await api.post(`/cameras/biometrics/${id}/add_relocation/`, {
      ...
      showNotification('Biometric successfully relocated and logs updated');
      setEditMode(false);
      fetchDetails();
    } catch (err) {
      console.error(err);
      showNotification('Failed to relocate device', 'error');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const downloadExcel = () => {
    const filename = `BIOMETRIC_ASSET_${device.name || 'Export'}_${new Date().toISOString().split('T')[0]}.csv`;
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] space-y-4">
        <div className="relative">
          ...
          <div className="absolute top-0 w-16 h-16 border-4 border-rose-500 rounded-full border-t-transparent animate-spin"></div>
        </div>
        <p className="text-dim font-bold animate-pulse text-xs uppercase tracking-widest">Scanning Identity Data...</p>
      </div>
    );
  }
  ...
  if (error || !device) {
    return (
      <div className="max-w-md mx-auto mt-20 glass-panel p-8 text-center border-red-500/20">
        ...
        <h3 className="text-xl font-bold text-main mb-2">Access Denied</h3>
        <p className="text-dim text-sm mb-8 leading-relaxed">{error || 'Device not found.'}</p>
        <button onClick={() => navigate('/biometrics')} className="w-full glass-button py-3 flex items-center justify-center">
          <ArrowLeft size={18} className="mr-2" /> Return to Identity Center
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in pb-12 relative">
      ...
      <div className="hidden print-only-header">
        ...
        <div className="letter-sub text-black">Identity & Access Management Protocol</div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 no-print">
        <div className="space-y-1">
          <nav className="flex items-center space-x-2 text-[10px] font-black uppercase tracking-[0.2em] text-dim">
            <button onClick={() => navigate('/dashboard')} className="hover:text-rose-400 transition-colors">COMMAND</button>
            <ChevronRight size={10} />
            <button onClick={() => navigate('/biometrics')} className="hover:text-rose-400 transition-colors">IDENTITY</button>
            <ChevronRight size={10} />
            <span className="text-rose-400">DETAIL VIEW</span>
          </nav>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button 
            ...
            className={`flex items-center space-x-3 px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all shadow-xl ${
              editMode ? 'bg-amber-500 text-black border-amber-400' : 'bg-rose-600 text-white hover:bg-rose-500 border-rose-400/50'
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <IntelligenceCard 
          icon={Clock} 
          label="Last Check-in" 
          value={device.lastCheckIn || 'Just now'} 
          color="amber" 
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="glass-panel overflow-hidden border-white/10 shadow-2xl bg-card/30">
              <div className="p-5 bg-panel border-b border-white/10 flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h3 className="text-sm font-black text-main uppercase tracking-widest">Identity Intelligence Registry</h3>
                    <p className="text-[9px] text-dim uppercase tracking-[0.3em] font-bold">Deep Hardware & Access Signature</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-8 space-y-12">
                <section>
                  <div className="flex items-center space-x-4 mb-6">
                    <p className="text-[10px] font-black text-rose-500 uppercase tracking-[0.3em] whitespace-nowrap">Hardware Identity</p>
                    <div className="h-px bg-rose-500/10 flex-1"></div>
                  </div>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-panel overflow-hidden border-rose-500/20 shadow-xl flex flex-col bg-rose-500/5">
            <div className="p-5 bg-rose-500/10 border-b border-rose-500/20 flex items-center justify-between">
               <div className="flex items-center space-x-3 text-rose-400 font-black text-[10px] uppercase tracking-widest">
                  <History size={16} />
            ...
            <div className="p-5 space-y-6">
               {unifiedActivity.length > 0 ? (
                 <div className="space-y-6 relative">
                   <div className="absolute top-2 bottom-2 left-[11px] w-px bg-rose-500/20"></div>
                   {unifiedActivity.slice(0, 3).map((log, i) => (
                     <div key={i} className="flex items-start space-x-4 relative group/item">
                        <div className={`w-6 h-6 rounded-full bg-panel border-2 ${
                          log.type === 'move' ? 'border-emerald-500' : 'border-rose-500'
                        } flex-shrink-0 z-10 flex items-center justify-center shadow-lg transition-transform group-hover/item:scale-110`}>
                           {log.type === 'move' ? <MapPin size={10} className="text-emerald-500" /> : <MessageSquare size={10} className="text-rose-500" />}
                        </div>
               ...
               {unifiedActivity.length > 0 && (
                 <button 
                   onClick={() => navigate(`/devices/history/biometric/${id}`)}
      ...
      <style dangerouslySetInnerHTML={{ __html: `
            @media print {
              ...
              .text-main, .text-dim, .text-rose-500, .text-secondary { color: #000 !important; }
              .border-rose-500\\/10 { border-color: #000 !important; margin-top: 30px !important; margin-bottom: 15px !important; }
...
function DetailRow({ label, value, mono = false }) {
  return (
    <div className="bg-panel/40 border border-white/5 p-4 rounded-2xl hover:bg-panel/60 transition-all group">
      ...
      <p className={`text-sm font-bold ${mono ? 'font-mono text-rose-300' : 'text-main'} truncate`}>{value || '—'}</p>
    </div>
  );
}
...
function IntelligenceCard({ icon: Icon, label, value, color = "blue", mono = false, action = null }) {
  ...
  return (
    <div 
      onClick={action}
      className={`glass-panel p-5 border-l-4 ${color === 'blue' ? 'border-blue-500' : color === 'emerald' ? 'border-emerald-500' : color === 'amber' ? 'border-amber-500' : color === 'rose' ? 'border-rose-500' : 'border-purple-500'} bg-card border-main shadow-lg hover:translate-y-[-4px] transition-all ${action ? 'cursor-pointer hover:bg-panel' : ''}`}
    >
